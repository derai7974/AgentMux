...
4. Identify core styling files in `{project_dir}` first (for example `tailwind.config.js`, `theme.ts`, or global CSS variables) so colors, spacing, and typography match the existing standard.
