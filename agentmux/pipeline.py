...
from typing import Any, Callable  # noqa: F401 — used by watchdog type stub
...
def orchestrate(
    ...
    dispatcher.register(CreatedFilesLogListener(files.created_files_log).handle_event)
    seed_existing_files(files.feature_dir, dispatcher)

    observer = Observer()
    observer.schedule(
        FeatureEventHandler(files.feature_dir, dispatcher), str(files.feature_dir), recursive=True
    )
    observer.start()
